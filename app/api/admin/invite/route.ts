...
async function sendMailjetEmail(to: string, token: string) {
  const apiKey = process.env.MAILJET_API_KEY!
  const secretKey = process.env.MAILJET_SECRET_KEY!
  const from = process.env.MAILJET_FROM_EMAIL || 'info@kundaliniyogatribe.de'
  ...
  await fetch('https://api.mailjet.com/v3.1/send', {
    ...
    body: JSON.stringify({
      Messages: [{
        From: { Email: from, Name: 'Kundalini Yoga Tribe' },
        To: [{ Email: to }],
...
export async function POST(req: NextRequest) {
  ...
  if (existingUser) {
    ...
    await sendMailjetEmail(email, token)
