...
export async function POST(req: NextRequest) {
  const { email, lang = 'de' } = await req.json()

  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    return NextResponse.json({ error: lang === 'en' ? 'Invalid email address' : 'Ungültige E-Mail-Adresse' }, { status: 400 })
  }
  ...
  if (!apiKey || !secretKey) {
    return NextResponse.json({ error: lang === 'en' ? 'Configuration error' : 'Konfigurationsfehler' }, { status: 500 })
  }
  ...
  const htmlPart = lang === 'en' ? `
    <div style="font-family: Georgia, serif; max-width: 560px; margin: 0 auto; padding: 40px 24px; color: #2C2416;">
      ...
      <h1 style="font-size: 28px; font-weight: 300; margin: 0 0 24px;">Almost there — confirm your email</h1>
      <p style="font-size: 16px; line-height: 1.7; color: #6B5D4F;">You signed up for the 7-Day Kundalini Reset. Click the button below to confirm your email address and we'll get started.</p>
      ...
      <p style="font-size: 13px; color: #9B8E7E; line-height: 1.6;">If you didn't sign up, you can safely ignore this email. The link expires in 24 hours.</p>
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: lang === 'en' ? 'Could not send email' : 'E-Mail konnte nicht gesendet werden' }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (e) {
    console.error('Subscribe error:', e)
    return NextResponse.json({ error: lang === 'en' ? 'Server error' : 'Server-Fehler' }, { status: 500 })
  }
}
