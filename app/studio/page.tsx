...
export const metadata = { title: 'Studio – KYT Admin' }
...
function Gate({ loggedIn }: { loggedIn: boolean }) {
  return (
    <div style={{
      ...
      <div style={{
        maxWidth: 520, width: '100%', background: C.panel,
        border: `1px solid ${C.border}`, borderRadius: 14, padding: 32, textAlign: 'center',
      }}>
        ...
        <p style={{ color: C.textSoft, fontSize: 14, lineHeight: 1.8, margin: '0 0 24px' }}>
          {loggedIn
            ...
            : 'Bitte melde dich mit einem Admin-Konto an.'}
        </p>
        {!loggedIn && (
          <SignInButton mode="redirect" forceRedirectUrl="/studio" fallbackRedirectUrl="/studio">
            <button style={{
              background: C.accent, color: '#000', border: 'none', borderRadius: 6,
              padding: '10px 18px', fontFamily: 'monospace', fontSize: 13, fontWeight: 700, cursor: 'pointer',
            }}>
              Anmelden
            </button>
          </SignInButton>
...
export default async function StudioPage() {
  ...
  if (!isAdmin) return <Gate loggedIn={!!userId} />
  return <StudioClient />
}
